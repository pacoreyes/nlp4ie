...
class_0 = [item for item in dataset_remapped if item["class"] == "0"]
class_1 = [item for item in dataset_remapped if item["class"] == "1"]
class_2 = [item for item in dataset_remapped if item["class"] == "2"]
...
class_0_training = class_0[:int(len(class_0) * 0.8)]  # 48 datapoints / 80%
class_0_validation = class_0[int(len(class_0) * 0.8):int(len(class_0) * 0.9)]  # 6 datapoints / 10%
class_0_test = class_0[int(len(class_0) * 0.9):]  # 6 datapoints / 10%

class_1_training = class_1[:int(len(class_1) * 0.8)]  # 48 datapoints / 80%
class_1_validation = class_1[int(len(class_1) * 0.8):int(len(class_1) * 0.9)]  # 6 datapoints / 10%
class_1_test = class_1[int(len(class_1) * 0.9):]  # 6 datapoints / 10%
...
for datapoint in tqdm(dataset_training, desc=f"Processing {len(dataset_training)} datapoints"):
  ...
  if FOR_OPEN_AI:
    ...
    ron_anonym = {"prompt": ron_anonym["text"], "completion": ron_anonym["label"]}

  # Save datapoint to JSONL files
  save_row_to_jsonl_file(row, output_dataset_training)
  save_row_to_jsonl_file(ron_anonym, output_dataset_training_anonym)
...
for datapoint in tqdm(dataset_validation, desc=f"Processing {len(dataset_validation)} datapoints"):
  ...
  save_row_to_jsonl_file(ron_anonym, output_dataset_validation_anonym)
...
for datapoint in tqdm(dataset_test, desc=f"Processing {len(dataset_test)} datapoints"):
  ...
  save_row_to_jsonl_file(ron_anonym, output_dataset_test_anonym)
