...
# random.shuffle(dataset_anonym)
dataset_anonym = dataset_anonym[:20]
...
# Process each text
for idx, datapoint in tqdm.tqdm(enumerate(dataset), desc=f"Processing {len(dataset)} datapoints", total=len(dataset)):
  idx = int(idx)
  ...
  for i in range(len(sentences)):
    ...
    if len(tokenizer.tokenize(" ".join(window))) > 510:  # Saving 2 tokens for padding
      ...
      # Find anonymized text
      row_anonym = [datapoint for datapoint in dataset_anonym if datapoint['id'] == row['id']]

      save_row_to_jsonl_file(row_anonym[0], temp_anonym)
