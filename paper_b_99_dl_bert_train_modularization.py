...
def run_training(parameters):
  ...
  """
    ...
    for hyperparameters in hyperparameters_combinations:
        ...
        data_file = parameters.get("dataset_path")
        ...
        model = BertForSequenceClassification.from_pretrained(parameters.get("bert_model"),
                                                              num_labels=len(LABEL_MAP),
                                                              hidden_dropout_prob=DROP_OUT_RATE)
        ...
        for i, batch in enumerate(tqdm(test_dataloader, desc="Testing")):
            with torch.no_grad():
                ...
                b_input_ids, b_attention_mask, b_labels = b_input_ids.to(device), b_attention_mask.to(
                    device), b_labels.to(device)
        ...
        confusion_matrix_filename = (
            f"{parameters.get('confusion_matrix_name')}_"
        ...
        losses_plot_filename = (
            f"images/{parameters.get('training_and_validation_losses')}_"
            f"lr_{LEARNING_RATE}_batch_{BATCH_SIZE}_warmup_{WARMUP_STEPS}_epochs_{NUM_EPOCHS}"
            f"_wd_{WEIGHT_DECAY}_dropout_{DROP_OUT_RATE}.png"
        )
